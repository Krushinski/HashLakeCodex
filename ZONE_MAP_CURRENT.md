...
Phase 108 keeps the single ordered zone table and pushes Zones 2-5 toward the reference image ecology read: a warmer, raised foreground shore; richer hill/valley texture on the green land; more recognizable 3D shoreline conifers; fuller spruce/canopy forms through the middle ground; and a denser foothill forest belt that visually climbs into the mountain base without creating a separate fake wall. It continues reducing pure-black needle silhouettes so the forest reads more like layered woodland and less like toy spires. The intent is foreground, middle ground, background, mountain base, not flat paper bands.
