...
Phase 110 keeps the single ordered zone table and pushes Zones 2-5 farther toward the reference image ecology read: a stronger foreground-to-background ground ramp; less contrast between visible land ribbons; visible shoreline spruce/sapling signatures; fewer pure-black needle silhouettes; brighter forest material response under moody light; and a fuller canopy/understory/foothill mix that visually climbs into the mountain base without creating a separate fake wall. The intent remains foreground, middle ground, background, mountain base, not flat paper bands.
...
- Expected elevation: `2.72 -> 3.72+`, with validated tree/canopy instances allowed to climb higher into the mountain-base transition.
